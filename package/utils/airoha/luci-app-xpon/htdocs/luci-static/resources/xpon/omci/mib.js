...
function load() {
	return uci.load('xpon').then(function() {
		var device = configuredDevice();
		return Promise.all([ rpc.omciClassList(device), rpc.omciMibList(device) ]);
	});
}
...
function render(group, data, title, description) {
	...
	function reloadObjects() {
		return rpc.omciMibList(configuredDevice()).then(function(result) {
			objects = result || [];
			renderTable();
		});
	}
			...
			E('div', { 'class': 'right' }, [
				...
				E('button', {
					...
					'click': function() {
						...
						return rpc.omciMibSet(configuredDevice(), classId, entityId, mask, hex).then(function() {
							ui.hideModal();
		...
		ui.showModal(_('Delete MIB object'), [
			...
			E('div', { 'class': 'right' }, [
				...
				E('button', {
					...
					'click': function() {
						return rpc.omciMibDelete(configuredDevice(), object.class_id, object.entity_id).then(function() {
							ui.hideModal();
	...
	return E([], [
		E('h2', {}, title),
		E('p', {}, description),
		E('p', {}, _('Object origin is reported explicitly by the Linux OMCI UAPI; no userspace inference is used.')),
